...
def generate_input(N, K, Q, seed, x_range=(1, 10000), y_range=(1, 10000), d_range=(1, 10), r_range=(0.8, 1.2)):
    random.seed(seed)
    # 配送センターと顧客の座標を生成
    ...
    for i in range(N + 1):
        for j in range(N + 1):
            if i != j:
                ...
                cost_matrix[i][j] = round(distance * r)
...
if __name__ == "__main__":
    ...
    # 各テストケースについて入力データを生成しファイルに保存
    for i, seed in enumerate(seeds):
        input_data = generate_input(N, K, Q, seed)
        filename = f"input_case_{i+1}.txt"
        save_input_to_file(filename, input_data)
        print(f"入力データが '{filename}' に生成されました。")
