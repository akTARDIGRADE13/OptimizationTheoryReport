...
Solution SimulatedAnnealingSolver7::solve(const Problem &problem) {
    Solution solution;
    ...
    while (nowTime < totalTime) {
        ...
        if (rng.randint(0, 100) < BORDER) {
            ...
            // 荷物swap
            int a = rng.randint(1, M + 1), b = rng.randint(1, M + 1);
            while (a == b) {
                b = rng.randint(1, M + 1);
            }

            std::swap(currentPerm[startTime[a]], currentPerm[startTime[b]]);
            std::swap(currentPerm[goalTime[a]], currentPerm[goalTime[b]]);

            // 評価
            Cost nextCost = calcCost(currentPerm, problem, M, costs, cargos,
                                    std::min(startTime[a], startTime[b]));
            ...
            if (acceptanceProb(currentCost, nextCost, temp) > rng.rnd()) {
                currentCost = nextCost;
                accepted++;
                std::swap(startTime[a], startTime[b]);
                std::swap(goalTime[a], goalTime[b]);
            } else {
                std::swap(currentPerm[startTime[a]], currentPerm[startTime[b]]);
                std::swap(currentPerm[goalTime[a]], currentPerm[goalTime[b]]);
                calcCost(currentPerm, problem, M, costs, cargos,
...
Coordinate SimulatedAnnealingSolver7::getPoint(const std::vector<int> &perm,
...
Cost SimulatedAnnealingSolver7::calcCost(const std::vector<int> &perm,
